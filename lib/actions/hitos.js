...
export async function createHito(data) {
  const hito = await prisma.hito.create({ data });
  revalidatePath('/workspace');
  revalidatePath('/metas');
  return hito;
}

export async function toggleHitoDone(id) {
  const hito = await prisma.hito.findUnique({ where: { id }, select: { done: true } });
