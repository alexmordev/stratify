...
export async function moveTarea(id, day, scheduledDate) {
  return prisma.tarea.update({ where: { id }, data: { day, scheduledDate: scheduledDate ?? null } });
}
...
export async function scheduleTarea(id, day, start, scheduledDate) {
  return prisma.tarea.update({ where: { id }, data: { day, start, scheduledDate: scheduledDate ?? null } });
}

export async function unscheduleTarea(id) {
  return prisma.tarea.update({ where: { id }, data: { day: null, start: null, scheduledDate: null } });
}

export async function unscheduleObjetivoTareas(objId) {
  return prisma.tarea.updateMany({
    where: { objId, day: { not: null }, done: false },
    data: { day: null, start: null },
  });
}

export async function changeTareaObjetivo(id, newObjId) {
  const tarea = await prisma.tarea.findUnique({ where: { id }, select: { objId: true, done: true } });
  if (!tarea || tarea.objId === newObjId) return;
  ...
  if (tarea.done) {
    await prisma.$transaction([
      prisma.objetivo.update({ where: { id: tarea.objId }, data: { done: { decrement: 1 } } }),
      prisma.objetivo.update({ where: { id: newObjId }, data: { done: { increment: 1 } } }),
    ]);
  }
