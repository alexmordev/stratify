...
export async function toggleObjetivoCompleted(id) {
  const obj = await prisma.objetivo.findUnique({ where: { id }, select: { completed: true } });
  const updated = await prisma.objetivo.update({ where: { id }, data: { completed: !obj.completed } });
  revalidatePath('/workspace');
  revalidatePath('/objetivos');
  return updated;
}
